...
let svg_dtd = Some "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.1//EN\" \"http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\">"
...
let to_attribute_tuple (attr : attribute) =
  ((attr.ns, attr.name), attr.value)


let rec to_svg_tree_frag (n : node) : node Xmlm.frag =
  `El (
      (
        (n.ns, n.name),
        List.map to_attribute_tuple n.attributes
      ),
      n.children
    )

let make_attribute_ns ns name value =
  {
    ns = ns;
    name = name;
    value = value;
...
let make_attribute name value =
  make_attribute_ns "" name value
...
let make_svg childs =
  {
    ...
    name = "svg";
    attributes = [
      make_attribute "version" "1.1";
      make_attribute_ns Xmlm.ns_xmlns "xmlns" "http://www.w3.org/2000/svg";
      make_attribute_ns Xmlm.ns_xmlns "xlink" "http://www.w3.org/1999/xlink"
    ];
    children = childs;
  }
...
let () =
  let file = match Array.length Sys.argv with
    | 2 -> Sys.argv.(1)
    | _ -> invalid_arg "Expected one SVG file argument"
  in
  let dst = open_out file in
  let o = Xmlm.make_output ~nl:true ~indent:(Some 2) (`Channel dst) in
  Xmlm.output_doc_tree to_svg_tree_frag o (svg_dtd, (make_svg []))
